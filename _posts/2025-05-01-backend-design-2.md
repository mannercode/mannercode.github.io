...
이번 시간에는 여러 유스케이스 중에서 절차가 복잡해 보이는 두 유스케이스인 `상영시간 생성하기`와 `티켓 구매하기` 중 `상영시간 생성하기`를 먼저 분석해 보도록 하겠다. `티켓 구매하기`는 이후 글에서 다룬다.
...
### 5.1. 마이크로서비스 아키텍처(MSA) 소개

서비스 설계를 시작하기 전에 프로젝트와 개발팀의 상황을 고려해서 최상위 아키텍처를 정해야 한다. 여기서는 마이크로서비스 아키텍처(MSA)를 선택했다.

MSA의 가장 큰 특징은 작은 서비스들이 협력해서 서비스를 제공한다는 것이다. 이때 중요한 점은 서비스 간에 DB를 공유하지 않는다는 것이다.
...
Frontend -> Gateway: 상영시간 생성 요청\nPOST /showtime-creation/showtimes
    ...
    Gateway -> ShowtimesService: createShowtimes(createDto)
        ...
        ShowtimesService -> ShowtimesService: saveShowtimes(createDto)
    Gateway <-- ShowtimesService: showtimes[]
...
위 시퀀스 다이어그램에서 볼 수 있듯이 서비스가 다른 서비스를 참조할 수 있다는 것은 기능이 확장되면서 언젠가 순환 참조 관계로 발전할 수 있다는 뜻이다.

### 5.4. SoLA(Service-oriented Layered Architecture)

SoLA는 ‘서비스 간 협력’과 ‘순환 참조 금지’ 사이의 상충 문제를 레이어 분리로 해결한다. 즉, 마이크로서비스는 하위 레이어만 참조할 수 있다는 `단방향 의존 관계` 규칙을 추가한다.
...
- **Application Service**:
  - 사용자 시나리오를 조립한다 (예: 주문 → 결제 → 알림).
  - Core/Infra 서비스 호출만 허용한다.
  - 트랜잭션 관리를 주도한다.
- **Core Service**:
  - 도메인의 기본 로직을 담당한다 (예: 영화 관리, 극장 관리).
  - Infra 서비스 호출만 허용한다.
- **Infrastructure Service**:
  - 결제, 스토리지 등 외부 시스템 연동을 담당한다.
...
최종 설계에는 아직 티켓 생성과 상영시간 충돌 검증이 빠져있다. 다음 글에서는 `상영시간 생성하기`의 부족했던 설계를 마저 보완할 것이다. 테스트는 그 이후 글에서 다룬다.

---

이전 글: [백엔드 서비스 분석과 설계 (1)]({% post_url 2025-04-01-backend-design-1 %})
