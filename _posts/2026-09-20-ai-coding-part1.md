---
layout: post
title: "AI 코딩 이해하기: AI 코딩이 바꾼 개발 환경"
lang: ko
...
- **AI 코딩**: 사람이 요구사항을 설명하고 AI에게 코드 작성·수정을 맡기는 방식

1편에서는 나를 포함한 참여자들이 어떤 입장에서 일했고 AI 코딩을 하면서 무엇을 겪었는지 돌아본다. 2편에서는 구체적인 기술 사례를 살펴보고 문제의 원인과 해결 방법을 모색한다.
...
<img src="{{ '/aicode/part1/attachments/nearly-finished-leaking-house.png' | relative_url }}" alt="완공을 앞뒀지만 천장에서 물이 새는 집" width="440" loading="lazy" style="max-width:100%;height:auto;">
...
package "사용자 앱·전용 단말 5개" {
    component "iOS 사용자 앱\n(iOS 네이티브)" as UserAppIOS
    component "Android 사용자 앱\n(Android 네이티브)" as UserAppAndroid
    component "입장 키오스크\n(iOS 네이티브)" as EntranceKiosk
    component "POS\n(Android 네이티브)" as Pos
    component "발권 키오스크\n(Android WebView)" as TicketKiosk
}

package "서버 애플리케이션 3개" {
    component "서비스 백엔드\n(Java)" as Backend
    component "발권 화면 서버\n(JSP)" as TicketServer
    component "백오피스\n(JSP)" as Backoffice
}
...
UserAppIOS --> Backend : REST API
UserAppAndroid --> Backend : REST API
...
잠시 자리를 비웠던 대표가 돌아오자 팀장은 그와 회의실로 갔다. 얼마 뒤에는 대표가 나를 불렀다.
...
나는 의인불용 용인불의(疑人不用 用人不疑)를 이야기하며, 사람이 의심스러우면 쓰지 말고 쓰려면 의심하지 말라고 했다. 말 한마디에 이렇게 흔들리면 앞으로 어떻게 함께 일하겠느냐고도 말했다.

대표는 Java를 할 수 있느냐고 다시 물었다. 나는 익숙하지 않은 기술이라도 금방 적응할 수 있고, 그동안 어떤 기술로 무슨 일을 해 왔는지는 이력서에서 확인할 수 있다고 답했다.

프로젝트를 수습하려면 무엇보다 신뢰 관계를 쌓는 게 중요해 보였다.

## 2. AI가 개발 방식을 바꾸다

### 2.1. AI로 기존 시스템을 분석하다
...
시스템의 전반적인 기능을 파악하는 데 하루가 걸렸다. 내 경험에 비춰 보면, 파일을 하나씩 열어 같은 작업을 했다면 일주일가량은 걸렸을 것 같았다.
...
적어도 이번 백오피스의 화면과 기본 구조는 요구사항을 명확히 전달하자 빠르게 구현됐다.

### 2.3. 백엔드 개발자에게 백오피스까지 맡기다
...
<figure>
  <a href="{{ '/aicode/part1/attachments/figma-user-app-design.png' | relative_url }}">
    <img src="{{ '/aicode/part1/attachments/figma-user-app-design.png' | relative_url }}" alt="Figma 캔버스에 배치된 사용자 앱 화면 시안" width="1780" height="1586" loading="lazy" style="width:100%;height:auto;">
  </a>
...
앞서 팀원에게 결과를 확인했느냐고 물었던 나도, AI가 계속 잘해 주자 같은 확인을 생략하고 있었다. 작업이 끝났다는 보고와 기능이 제대로 동작한다는 확인은 다른 일이었다.
...
<figure>
  <a href="{{ '/aicode/part1/attachments/github-projects.png' | relative_url }}"><img src="{{ '/aicode/part1/attachments/github-projects.png' | relative_url }}" alt="요구사항을 Todo, In progress, Done으로 나눈 GitHub Projects 작업 보드" width="960" loading="lazy" style="max-width:100%;height:auto;"></a>
...
그러나 내가 백오피스와 Flutter 앱을 AI 코딩으로 만들며 경험한 업무량은 이전과 달랐다. iOS와 Android라는 플랫폼의 차이는 있었지만 구현할 기능은 상당 부분 같았다. 공통 요구사항을 정리한 뒤 플랫폼별 구현을 AI에게 맡기면, 사람이 모든 구현을 처음부터 반복해야 하는 부담은 줄일 수 있다고 판단했다.
...
더 부드럽게 말해야 한다는 것은 알고 있었다. 그러나 회사 자금이 거의 바닥난 상황에서 누군가 업무에 적응할 때까지 기다릴 여유가 없다고 생각했다.

### 4.3. 검증 없는 완료 보고가 반복되다

앞서 백오피스 API를 연동할 때 겪었던 문제는 이후에도 반복됐다. 구현됐다는 기능을 확인하면 오류가 나왔고, 수정을 요청한 뒤 다시 확인하는 일이 이어졌다.
...
AI가 작업을 끝내도 결과를 검증하지 않았고, AI에게 어떻게 검증할지 묻지도 않았다. 매출 통계가 계속 맞지 않는다면 최소한 검증 계획을 요구하고, 그 계획이 타당한지는 검토했어야 했다.

나는 지난번 백오피스 API를 연동할 때 충분히 주의를 줬다고 생각했다. 그러나 이번에도 변경 결과와 기존 기능을 확인하는 과정은 빠져 있었다. 나는 이를 결과를 검증할 책임을 받아들이지 않는 태도로 봤다.
...
처음에는 내가 참으면 감당할 수 있는 문제라고 생각했다. 그러나 이번에는 검증하지 않은 결과 때문에 회사와 다른 구성원들까지 피해를 보고 있었다.

그 말을 들은 다음 날 해고를 통보했다.

그에게 일을 맡겨도 결과를 확인하고 수습하는 일은 계속 다른 사람들의 몫이었다.
...
나는 대표에게 당분간 채용을 중단하고 혼자 개발하는 게 낫겠다고 제안했다. 그때는 몰랐지만, 대표는 개발팀이 무너져 사업을 접어야 하는 것은 아닌지 걱정했다고 한다. 그러나 팀원들이 실제로 해내던 일과 그 과정에 내가 쏟던 시간을 따져 보면, 당시에는 단독 개발이 최선의 선택이라고 판단했다.
...
Java와 Android는 익숙하지 않았지만 부담스럽지 않았다. 당시 맡은 작업에서는 구현할 요구사항을 알고 있었고, 말로 지시하고 결과를 검증하는 방식도 같았다. 기본 개념을 이해하고 모르는 세부 사항은 AI에게 물으면서 작업을 이어 갔다.
...
프로젝트와 사람에 따라 다르겠지만, 당시 내게는 네 개가 거의 한계였다. 한 에이전트의 결과를 확인하고 나면 다른 에이전트가 검증을 기다리고 있었다. 동시에 일을 시킬 수 있는 수보다 내가 결과를 확인할 수 있는 속도가 한계를 정했다.
...
개발 경험이 없던 사람이 한 달 만에 그만큼의 화면과 기능을 AI 코딩으로 구현한 것은 놀라웠다. 하지만 화면과 기능이 많다는 이유만으로 운영 가능한 상태라고 판단할 수는 없었다. Supabase는 나도 이름만 들어 봤을 뿐이어서, 어떤 서비스인지부터 알아봐야 했다.

무엇보다 한 달 동안 내게 전혀 말하지 않고 혼자 진행했다는 게 화가 났다.

> 이 글을 쓰다가 대표에게 전화해서 이유를 물었다. 당시에는 개발자들을 믿지 못해서 그랬다며, 자세한 해명은 다음 기회에 하겠다고 한다. 그렇더라도 함께 개발할 나에게 왜 진행 상황을 공유하지 않았는지는 이해하기 어려웠다.
...
대표는 사소한 문제들이고 자신이 해결하겠다고 했다. 하지만 내가 보기에는 코드 몇 군데를 고치는 수준을 넘어, 실제 운영 흐름을 다시 확인해야 하는 상태였다.

개발을 주도한 것은 대표였지만, 그 결과에 대한 책임은 내게 돌아올 것이라고 생각했다. 대표는 아니라고 했지만, 나 역시 대표를 믿지 못했다.

### 5.2. 기술 선택을 둘러싼 갈등

사전 협의가 없었다는 점만 문제는 아니었다. 내가 이 프로젝트에 적절하지 않다고 판단한 기술 스택으로 개발을 이어 가야 한다는 데 더 큰 거부감을 느꼈다.

대표는 AI가 추천한 Next.js와 Supabase 조합을 선택했다. Supabase는 데이터베이스와 인증, API 같은 백엔드 기능을 제공해 직접 코딩할 일을 줄여 준다.

대표는 어차피 AI 코딩을 할 것이었다. 필요한 코드를 만드는 부담이 줄어든 만큼, 코드 작성을 줄여 주는 편의의 가치도 다르게 봐야 한다고 생각했다. 이후 유지할 기술 구성과 서버 운영 비용까지 따져야 했다.

당시에는 필요한 서버 기능을 Next.js에 모으고 MongoDB를 사용하는 편이 구성을 단순하게 만들고 서버 운영 비용도 줄일 수 있다고 판단했다. 다만 비용상의 우위를 단정하려면 실제 기능 범위와 운영 조건을 맞춘 비교가 필요하다.

집필 시점에는 사용자 앱에서 간단한 데이터를 요청해도 응답을 받는 데 1초 정도가 걸리고 있었다. 성능을 측정했을 때는 Vercel에서 Supabase를 호출해 응답을 받는 구간에만 500ms 정도가 걸렸다.

한국 리전의 EC2 한 대에 Next.js와 Supabase 전체를 Docker로 함께 실행했을 때도 전체 응답에 약 1초가 걸렸다. 클라우드 서비스 간 리전 차이만으로 설명하기는 어려웠다.

다만 각 구성에서 어느 처리 과정이 지연을 만드는지까지 자세히 조사한 것은 아니었다. 이 측정값만으로 Supabase 자체가 원인이라고 단정할 수는 없다. 집필 시점에는 이 문제에 대응하기 위해 Supabase를 걷어내고 있다.

키오스크와 사용자 앱의 기술 선택에서도 의견이 갈렸다. 대표는 둘 다 React Native(RN)로 만들려 했고, 쉬운 배포를 중요한 조건으로 내세웠다.

그러나 이 프로젝트의 키오스크에서는 배포 편의보다 적은 메모리와 느린 CPU에서도 원활하게 동작하고, 프린터와 스캐너 같은 장치를 안정적으로 지원하는 것이 중요했다. Android만 지원하면 되는 키오스크에서 RN의 추가 실행 환경이 차지할 메모리와 하드웨어 연동의 복잡성은 부담이라고 판단했다.
...
내가 보기에는 기술 선택에 필요한 조건이 충분히 반영되지 않은 채 배포 편의가 앞서고 있었다. AI의 추천을 따르더라도 단말의 제약과 장치 연동, OS별 동작 같은 조건이 빠지지 않았는지 판단할 수 있어야 했다.

나는 이런 이유를 설명했다. 하지만 대표는 내 판단보다 AI의 제안을 더 신뢰했다. 내가 무슨 말을 해도 AI에게 다시 물어보고 답을 비교했다. 양쪽의 논리적 근거를 듣고 최종 판단은 자신이 하겠다는 것이었다.

이런 식으로 AI의 답변과 경쟁하며 대표를 설득해야 하는 상황은 처음이었다. 처음 만났을 때 믿고 맡겨 달라고 했지만, 이제는 내 판단과 AI의 답변이 나란히 비교되고 있었다.

논리적 근거의 타당성을 비교하는 데에도 전문성이 필요하다. 그 주장이 어떤 전제에서 성립하는지, 빠진 조건과 운영상의 위험은 무엇인지 판단할 수 있어야 한다.

> 삼성전자에서도 비슷한 의사결정 구조를 비판하는 목소리가 나왔다. [2024년 10월 동아일보 딥다이브 인터뷰](https://stibee.com/api/v1.0/emails/share/OfXIP-Gm1JSC7-0jDdf0nbQSQeFwMfU)에서 반도체 부문의 한 엔지니어는 당시 정현호 사업지원TF장에게 올리는 보고서를 ‘초등학생도 알아들을 수 있게’ 써야 했다고 말했다. 그는 기술적 이해가 부족한 경영진이 결정을 내리고, 보고가 올라가는 동안 현장에서 제기한 위험이 축소되는 구조를 비판했다.
>
> 이 사례에서 내가 주목한 것은 기술적 판단에 필요한 전문성과 최종 결정권이 어긋나 있다는 점이다.

<figure>
  <a href="https://stibee.com/api/v1.0/emails/share/OfXIP-Gm1JSC7-0jDdf0nbQSQeFwMfU"><img src="{{ '/aicode/part1/attachments/donga-deepdive-excerpt.png' | relative_url }}" alt="동아일보 딥다이브의 질문: 반도체 엔지니어가 초등학생 수준으로 내부 보고서를 쓴다고?" width="630" height="52" loading="lazy" style="max-width:100%;height:auto;"></a>
...
이후에도 대표는 중요한 기술적 결정을 직접 내렸다. AI의 길고 장황한 답변에서 내가 보기에는 말이 안 되는 주장까지 읽고 반박해야 했다. 이런 일을 반복하는 내 모습이 구차하게 느껴졌다. 퇴사하겠다는 말이 목구멍까지 올라와 간질였지만, 겨우 다시 삼켰다.

나는 대표가 개발자를 불신하게 된 데에는 일부 개발자의 오만한 태도도 큰 몫을 했다고 생각한다. 글 앞부분에서 전 개발팀장 이야기를 자세히 꺼낸 것도 그 배경을 보여 주기 위해서였다. 내가 합류하기 전부터 반복된 출시 실패도 함께 생각해야 했다.

### 5.3. 대표의 선택에도 의미는 있었다
...
<figure>
  <video controls playsinline preload="none" width="1232" height="736" style="width:100%;height:auto;aspect-ratio:1232/736;background:#111;" aria-label="하얀거탑 장면을 바탕으로 만든 영상: 수술은 여기서 내가 통제하겠네" aria-describedby="white-tower-caption">
    <source src="{{ '/aicode/part1/attachments/grok-video-58178a05-0934-4955-8c0e-e25f58ee072f.mp4' | relative_url }}" type="video/mp4">
    <a href="{{ '/aicode/part1/attachments/grok-video-58178a05-0934-4955-8c0e-e25f58ee072f.mp4' | relative_url }}">직접 만든 하얀거탑 영상 보기</a>
  </video>
...
나는 데이터 이관까지 포함해 4주가 필요하다고 했다. 그러나 대표는 내 의견을 받아들이지 않고 2주 안에 끝낼 것을 요구했다.

AI에게 구현을 맡기면 2주 안에도 만들 수는 있겠지만, 검증 시간이 부족해 사고가 날 가능성이 높다고 설명했다. 그 정도 완성도로는 미술관에서 불만을 제기할 것이라고도 했지만, 대표는 괜찮을 거라고 했다.

대표는 최종 책임자다. 위험을 설명했는데도 괜찮다고 하니, 그 위험을 감수하겠다는 판단으로 받아들이고 일정에 맞춰 진행하기로 했다. 하지만 문제가 생기면 책임은 내게 돌아올 것이라고 생각했다.
...
일정을 서두른 것이 중요한 원인이라고 생각했지만 대표에게 다시 말하지는 않았다. 구차하게 핑계를 대고 싶지 않았다. 대표가 개발을 주도하면서도 결과에 대한 책임은 개발자가 주도하던 때의 기준으로 묻는다고 느꼈다.

### 5.5. 당장 쓰지 않을 기능을 계속 추가하다

대표가 AI로 계속 기능을 추가하면 나는 그에 맞춰 후속 작업을 해야 했다. 레거시를 수습할 때처럼 할 일이 명확한 상태에서 AI 코딩에만 집중하기가 어려워졌고, 일의 효율과 의욕이 급격히 떨어졌다.
...
당장 쓰지 않을 기능을 미리 만들어 놓으면, 나중에 새로운 기능을 추가할 때도 그 기능을 함께 살펴야 한다. 한 번 만들면 끝나는 것이 아니라 계속 기존 기능을 건드리게 된다.

티켓 발송에는 메시지를 보낼 때마다 미리 충전한 금액에서 요금을 차감하는 기능이 있었다. 그러나 서비스가 전반적으로 자동화된 상태는 아니어서 행사마다 개발자가 붙어야 했다. 메시지를 보내면 시스템상 요금은 차감됐지만, 실제로 의뢰업체에 요금을 청구한 적은 한 번도 없었다.

그런데 행사 진행 중 충전 잔액이 바닥나면서 서비스가 멈췄다. 원인을 바로 파악하지 못해 한동안 발권이 중단됐다. 의뢰업체에 요금을 청구하지도 않는 상태에서, 충전 잔액이 실제 발권을 막는 조건이 된 것이다.

여기에 기존 고객에게 안내 문자를 보내는 기능을 추가한다고 해 보자. 이 메시지에도 요금을 차감할 것인지 정해야 한다. 차감하는 경우와 하지 않는 경우가 모두 있다면 이를 선택하는 옵션이 필요하고, 각 경우에 요금 차감이 제대로 적용되는지도 검증해야 한다.

메시지 발송에 붙인 과금 기능 하나가 문자 발송 전체에 영향을 주는 것이다. 기능은 이렇게 서로 연결돼 있다. 체계적으로 개발하지 않으면 기능 하나를 추가할 때 기존 기능을 더 많이 고쳐야 한다.

기능이 늘면서 개발과 유지보수에 더 많은 인력이 필요해지면, 그 비용을 감당할 매출도 필요하다. 그런데 당장 쓰지 않을 기능을 미리 만들면 매출에 기여하기도 전에 유지할 부담부터 늘어난다.

이쯤 되니 대표를 설득해 함께 개발 방향을 정하기 어렵다고 느꼈다. 나는 구현 결과에 맞춰 뒤따라가고 있었고, 기술적 우려를 말해도 방향을 바꾸기는 어려웠다.

나는 고액의 연봉을 받으면서도 능력을 발휘하지 못한다고 느꼈다. 개발은 빠르게 진행됐지만, 시스템의 엔트로피도 그만큼 빠르게 증가하고 있었다.

> 엔트로피는 흔히 ‘무질서의 정도’로 설명하는 개념이다. 여기서는 정리하지 않고 물건만 쌓아 둔 방처럼, 구조를 정리하지 않은 채 기능만 붙여 시스템이 점점 얽히고 고치기 어려워지는 상황을 비유한다.

나는 대화로 상황을 바꾸기 어렵다고 판단하면 문제가 드러날 때까지 기다리는 편이다. 당시에도 검증 없이 기능을 계속 붙이면 수습하기 어려운 문제가 드러날 것이라고 생각했다. 대표가 직접 그 문제를 겪기 전에는 어떤 말을 해도 역효과만 날 것 같았다.

## 맺으며

8개월 동안 AI 코딩은 구현의 속도뿐 아니라 일하는 방식과 사람 사이의 관계까지 바꿨다. 내가 여러 애플리케이션을 혼자 다룰 수 있게 됐고, 대표는 생각한 서비스를 직접 만들어 개발의 방향까지 결정하게 됐다.

합류 당시의 개발팀장과 대표 사이에서도, 대표와 나 사이에서도 신뢰를 쌓는 데 실패했다. 서로의 판단을 조율하지 못하니 각자의 능력도 제대로 살리지 못했다. 구현이 빨라진다고 협업까지 나아지는 것은 아니었다.

불신을 키운 요인 중에는 일부 개발자의 오만한 태도도 있었다. 과거에는 필요한 기술을 가진 개발자를 대체하기 어려워 상대방이 맞춰 주는 일이 많았다. 그러나 이제는 코드 작성을 AI에게 상당 부분 맡길 수 있다. 코드를 빠르게 작성한다는 이유만으로 협업하기 어려운 개발자를 고집할 이유가 줄었다.
...
이 프로젝트에서 대표는 여러 아이디어를 빠르게 시도하며 가능성을 확인하려 했고, 나는 하나의 문제를 깊이 파고들어 제대로 해결하려 했다. 빠른 시도와 집요한 검증이 실제 서비스의 완성으로 이어지려면, 무엇을 만들고 무엇을 완료로 인정할지 함께 판단해야 한다. AI가 구현의 속도를 높인 만큼, 서로 다른 두 성향이 긴밀하게 협력하도록 만드는 일이 더 중요해졌다.

2편에서는 구체적인 기술 사례를 바탕으로, 이 경험에서 드러난 문제의 원인과 해결 방법을 정리하려고 한다.
