...
클래스를 사용하는 쪽에서는 `interface Language`만 알면 되고 그것이 `Korean`인지 `English`인지는 신경쓰지 않는다.\
...
좋은 코드를 작성하려면 객체지향에 대한 이해와 요구사항을 해석하는 능력이 필요하다.\
객체지향을 이해하지 못하면 설계를 하지 못하고 요구사항 해석을 하지 못하면 설계가 계속 흔들린다.\
설계가 흔들리면 비교적 큰 규모의 리팩토링을 자주 하게된다.\
그러면 테스트 자동화를 유지하기 어려운 것은 물론이고 객체들도 단순해져서 if-else가 넘쳐나는 절차식 비슷한 구조가 된다.\
결국 힘들게 공부한 디자인 패턴/UML/TDD/DDD 등등을 실제 프로젝트에 적용하기 어려울 것이다.

구현에 익숙해져있기 때문에 혹은 효율성을 이유로 설계를 구현에 맞추는 실수를 한다.\
이것은 습관이기 때문에 머리로 알고 있어도 어느 순간 그렇게 된다.\
그러나 철저하게 분석을 기반으로 설계와 구현을 해야한다.\
그렇지 않으면 요구사항과 코드의 개념과 의도가 일치하지 않아서 요구사항이 변경될 때 코드가 그것을 수용할 수 없게 된다.

객체지향이 어렵다고는 하지만 얼마간의 노력과 한 순간의 아이디어로 사고의 전환을 이룰 수 있다.\
요구사항 해석은 사고의 전환은 물론이고 그 후에도 많은 노력과 경험을 필요로 하는 만큼 몇 배는 더 어렵다.\
많은 프로그래머들이 이 어려움을 극복하지 못하고 노력을 중단하는 것을 보아왔다.\
그러나 요구사항 해석까지 도달하면 소프트웨어를 만들 떄 하게되는 막연한 고민과 답답함이 사라지면서 재미있는 코딩을 하게된다.

누군가 코딩을 배워야 하는 이유를 물었을 때 사람들은 논리적인 사고에 도움이 된다는 막연한 대답을 했다.\
내 생각에 코딩으로 얻을 수 있는 것은 문제의 본질이 무엇인지 통찰하고 해결하는 능력이다.
