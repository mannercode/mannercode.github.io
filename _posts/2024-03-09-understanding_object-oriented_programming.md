...
수 많은 글이나 영상에서 객체 지향 프로그래밍을 설명한다. 그러나 그런 자료들을 아무리 봐도 객체지향이 무엇인지 선뜻 이해하기가 쉽지 않다. 왜 그럴까?
...
흔히 이야기하는 캡슐화, 은닉화, 다형성 상속성 같은 원칙들은 위의 본질에서 도출된 개념일 뿐이다.
...
지금처럼 printDocument() 함수 하나만 변경하는 것이라면 문제가 되지 않는다. 그러나 printDocument() 외에 다른 함수가 있다면? 변경해야 하는 함수가 그 만큼 늘어난다.
