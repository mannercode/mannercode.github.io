...
if-else로 함수 마다 타입 체크를 하고 있다. 이 상황에서 HttpRequest를 HttpRequest2로 변경해야 한다면 어떨까?

실제 프로젝트는 이것보다 더 길고 복잡한 코드인 경우가 많다. 이런 환경에서 관련된 모든 함수를 찾아서 변경하는 것은 쉽지 않을 것이다.
...
> 코드에 타입을 판별하는 if 문이 보인다면 이것이 절차식은 아닌지? 객체 지향으로 개선할 수 없는지 고민해 보기를 바란다.
...
이것이 프로퍼티를 public으로 직접 노출하지 말라는 이유이다. 프로퍼티가 public이면 클래스 외부의 어떤 함수든 그 값을 읽고 변경할 수 있다. 외부에서 프로퍼티를 변경할 수 있다면 프로퍼티와 관련된 코드를 변경할 때 그 영향이 어디까지 미칠지 알 수 없게 된다. 잠재적으로 전역변수와 다를바 없게 되는 것이다.
