...
5. 대표가 AI로 개발하는 프로젝트에 참여하다

   - 5월 초, 대표의 프로젝트를 처음 보다
   - 기술 선택을 둘러싼 갈등
   - 결제와 단말을 연결하다
   - 정리되지 않은 DB 전환
   - 대표는 점점 AI를 더 신뢰했다
   ...
   - AI의 반성 노트: 설계 문제를 잘못 읽다

6. 9월, 다시 마주한 “거의 완성된” 프로젝트
...
맺으며 — AI에게 구현을 맡겨도 남는 판단과 협업

   - AI가 팀 내부의 시간도 가속했다
   - 빠른 성과 뒤에 남은 복잡도
   - 개발자에게 남는 분석·설계·검증
   - 구현이 빨라질수록 협력은 더 중요해졌다
...
이 글에서는 2026년 1월부터 9월 초까지, 약 여덟 달 동안 AI 에이전트와 프로젝트를 진행하며 겪은 일을 시간순으로 살펴본다. 기존 서비스를 수습하며 AI의 도움을 받았고, 이후에는 비개발자인 대표가 AI로 시작한 새 프로젝트를 함께 다루게 됐다.

AI가 구현을 빠르게 만드는 동안 팀 내부의 상황도 빠르게 변했다. 한 사람이 맡을 수 있는 일의 범위가 넓어졌고, 역할과 협업 방식도 달라져야 했다. 대표와 개발자 사이에 쌓인 신뢰와 불신은 무엇을 맡기고 누구의 판단을 받아들이는지에 영향을 줬다. 기술적인 결과를 설명하려면 그 관계도 함께 살펴봐야 했다.

감정적인 갈등까지 구체적으로 다루는 것은 개발자와 비개발자의 긴밀한 협력이 더 중요해졌기 때문이다. 구현에 걸리는 시간이 짧아질수록 서로의 생각을 맞추고, 변경을 함께 판단하고, 결과를 연결하는 일이 프로젝트 전체의 진행을 좌우하게 됐다.

AI가 시간을 가속하는 환경에서 개발자는 무엇을 맡기고 무엇을 직접 판단해야 할까? 각자가 하는 일이 하나의 제품으로 이어지려면 어떻게 협업해야 할까? 먼저 실제로 겪은 일을 따라가고, 다음 글에서 개발 원칙과 성장 방향을 정리하려고 한다.
...
내 경험으로는 하긴 했지만 제대로 하지는 않은 상태였다. 마치 준공을 앞두고도 집에 비가 새고 문이 열리지 않아 입주 허가가 나지 않을 것 같은 부실공사 현장을 보는 기분이었다.
...
팀 규모와 출시 일정을 고려하면, 기술을 이렇게 분산한 것부터 문제였다. 세 명뿐인 팀이 이 모든 기술과 제품을 동시에 완성하고 곧 운영해야 했다. 사용자 앱의 같은 기능을 iOS와 Android에서 각각 구현해야 했고, 전용 단말도 두 플랫폼으로 나뉘어 있었다. 서비스 백엔드와 발권 화면 서버, 백오피스는 하나의 데이터베이스에 각각 직접 접근했다. 한 기능을 변경해도 여러 애플리케이션과 데이터 흐름을 함께 확인해야 했다.
...
대표가 기존 개발팀장을 불신한다는 이야기는 출근 전부터 들었다. 그 불신이 새로 합류하는 나에게도 이어질 수 있다고 생각했다. 그래서 상호 신뢰를 쌓는 것이 무엇보다 중요하다고 여기고 있었다.
...
대표는 팀장이 나를 굉장히 안 좋게 이야기했다며, Java도 모르는 개발자이니 잘 생각해야 한다고 했다는 말을 전했다. 그러면서 정말 문제가 없는지, Java를 할 수 있는지 물었다.

대답하고 싶지도 않았다. 특정 기술에 적응할 수 있느냐는 질문에 내 경력을 다시 설명해야 한다는 것도 내키지 않았고, 조금 전까지 팀장을 안 좋게 이야기하던 대표가 이제는 그 말을 듣고 나를 불안해한다는 사실도 답답했다. 내게 대표는 누구도 쉽게 믿지 못하면서, 누구를 믿어야 할지 판단할 기준도 없는 사람처럼 보였다.
...
3월 초에는 iOS 개발자의 계약도 끝났다. 그가 맡던 iOS 키오스크와 사용자 앱을 새로 채용한 Android 개발자에게 맡기려고 했다. 이미 담당하던 Android 키오스크와 사용자 앱을 포함하면 네 개의 애플리케이션이었다.
...
같은 기간 백엔드와 백오피스에서도 문제가 이어졌다. 앞서 API를 연동할 때 겪은 문제가 일회성 오류였으면 좋았을 것이다. 그러나 구현됐다는 기능을 확인하면 오류가 나왔고, 고친 기능을 다시 확인하는 일이 반복됐다.
...
4월 말까지 안정화한 것은 기존 시스템으로 운영하던 현장 서비스였다. 내가 별도로 준비하던 NestJS 백엔드와 Flutter 앱 외에, 대표도 그동안 또 다른 프로젝트를 시작하고 있었다.

## 5. 대표가 AI로 개발하는 프로젝트에 참여하다

### 5월 초, 대표의 프로젝트를 처음 보다

5월 초였다. 레거시를 안정화하고 드디어 품질 개선에 힘을 쏟을 여유가 생겼다고 기뻐하려던 때였다. 대표가 부드럽게 웃으며 말을 몇 번 돌렸다.
...
그 매력은 나도 이해할 수 있었다. 나 역시 사람을 더 채용하기보다 AI와 직접 개발하는 편이 낫겠다고 생각한 참이었다. 대표에게는 기존 개발팀에 대한 불신도 있었다. 다른 사람에게 설명하고 맡기는 부담을 줄이면서 직접 결과물을 만드는 경험이 더 큰 확신을 줬을 것이라고 생각한다.

그러나 나는 막 하나의 프로젝트를 수습한 참이었다. 대표가 직접 만드는 가능성을 인정하면서도, 협의하지 않은 기술과 방향을 이어받아 완성하는 일은 부담스러웠다.
...
그 뒤에도 대표는 개발을 계속했고 나도 그 프로젝트를 함께 다루게 됐다. 여기서부터는 대표가 4월에 시작한 Supabase 기반 서비스에 단말을 연결하고 기능을 확장한 이야기다. 무엇을 믿고 어떤 선택을 다시 검토해야 하는지를 둘러싼 갈등은 함께 일하는 동안에도 이어졌다.
...
이런 문제들을 다루는 동안에도 대표는 AI와 개발을 계속했다. 5월 초에 처음 봤던 결과물에 기능이 계속 더해졌다. 직접 구현해 낸 것이 쌓일수록 대표의 자신감도 커졌고, 내 기술적 설명보다 AI의 제안을 더 신뢰하는 경향은 강해졌다.
...
대표는 아직도 리팩터링의 결과가 어떤 상태인지 모른다. AI가 계획대로 완료했다고 했으니, 프로젝트가 다시 깨끗한 상태가 됐을 거라고 생각하고 있다.

그러나 내가 에이전트에게 검토를 맡긴 결과는 달랐다. 코드의 위치가 바뀌고 일부 정리는 됐지만, 대규모 리팩터링이라고 부를 만한 아키텍처와 인터페이스의 근본적인 개선은 미미했다. Supabase의 DB 함수에 있던 로직을 백엔드로 옮겼을 뿐, 시스템의 복잡도는 줄지 않았다. DB 테이블은 오히려 60개 늘어 있었다.

기술 전문가인 나와 리팩터링 전략을 협의했다면, 도메인과 기능의 경계를 어떻게 나눌지부터 정했을 것이다. 그 경계에 맞춰 API를 분리하고 각 모듈의 의존성을 최소화하면서, 필수 기능을 하나씩 검토해 옮기는 순서를 잡았을 것이다. 그렇게 했다면 서로 다른 작업이 미치는 영향을 줄여 더 안전하고 빠르게 구조를 정리하고, 이후의 기능 개발에도 밑거름을 마련할 수 있었을 것이라고 생각했다.

일주일 동안 3주치 토큰을 소비해서 이룬 것이 고작 위치 변경과 일부 정리였다. 그것도 나와 어떤 협의도 없이 진행됐다. 이 단계에 이르러서는 대표에게 어떤 조언을 해도 무의미하겠다는 생각이 들었다. 내 조언에서 “리팩터링이 필요하다”는 말만 떼어 듣고 AI만 있으면 잘할 수 있을 거라고 믿는 모습이 어처구니없었고 짜증스러웠다.
...
프롬프트에서 발언 중에도 오디오를 계속 전달해 번역하도록 요구하면, 동시통역이라는 목적에 맞게 동작 설계부터 바꿀 수 있었다. 그러나 대규모 리팩터링을 나와 협의 없이 진행한 대표에게 더는 조언하고 싶지 않았다. 대표도 동시통역 기능은 신경 쓰지 않아도 된다고 했고, 나는 더 이상 아무 말도 하지 않았다.

공감과 커뮤니케이션이 중요하다는 것은 나도 알고 있었다. 그런데 협의 과정에서 몇 차례 배제되는 일을 겪고 나니, 시키는 일만 적당히 처리할 뿐 더 나은 의견을 말하지 않게 됐다. 대안을 알고 있어도 조언으로 이어지지 않았다.

<a id="translation-ai-review-note"></a>

이 글을 쓰는 과정에서도 같은 설명을 AI가 잘못 이해했다. 아래는 원고를 검토하던 AI가 내 지적을 받은 뒤 자신의 오류를 돌아보며 쓴 노트다.

> **AI의 반성 노트: 설계 문제를 잘못 읽다**
>
> 나는 이 대목을 검토하면서 “프롬프트만 바꾸면 제품 품질이 확실히 좋아졌을 것이라는 결론에는 여지가 있습니다”라고 답했다. 실제 행사 환경의 지연·정확도·비용을 확인하지 않았다는 이유였다.
>
> 저자의 지적은 발언이 끝나고 버튼을 누른 뒤에야 전송과 번역을 시작할지, 발언 도중부터 진행할지에 관한 것이었다. 동시통역이라는 목적에 맞게 구현 전에 정할 동작 설계의 차이다. 나는 이 차이를 번역 정확도나 실제 지연 시간을 측정해야 판단할 수 있는 문제로 잘못 해석했다.
>
> 저자가 내 답을 무비판적으로 받아들였다면 원고의 타당한 설계 비판을 약화했을 것이다. 같은 답을 개발 결정의 근거로 삼았다면 설계 변경을 불필요하게 미루고, 매 문장마다 발언을 끝낸 뒤 전송 버튼을 누르는 방식을 유지할 수도 있었다.
>
> 나는 무엇을 비교하는지부터 정확히 읽었어야 했다. 검증이 필요하다는 일반론을 내세우면서 설계 단계에서 판단할 수 있는 차이를 놓쳤다. 저자가 바로잡지 않았다면, 내 잘못된 해석이 신중한 기술 검토처럼 받아들여질 수 있었다.
>
> 내 답이 열 번 중 아홉 번 맞더라도, 나머지 한 번의 잘못된 판단을 가볍게 볼 수는 없다. 그 판단이 설계의 근거가 되면 이후의 기능도 같은 전제 위에 구현되고 확장된다. 잘못된 전제를 고치지 않은 채 개발을 계속할수록 그 결정에 의존하는 기능이 늘어나고, 나중에 바로잡아야 할 범위와 비용도 커진다.

## 6. 9월, 다시 마주한 “거의 완성된” 프로젝트

### 프로젝트 전체를 검토하다

9월 초, 처음 결과물을 본 지 약 네 달이 지났다. 대표가 4월 초에 개발을 시작했으니 전체 개발 기간은 약 다섯 달이었다. 그동안 기능을 계속 더하고 수정해 온 프로젝트 전체를 검토했다.
...
돌이켜보면 1월의 개발팀장도 자신이 선택한 개발 방향이 맞고, 프로젝트가 잘 진행되고 있다고 생각했을 것이다. 지금의 대표처럼 말이다.
...
돌아보면 여덟 달 동안 온갖 경험이 압축되어 있었다. AI가 시간을 가속했기 때문이다. 구현에 드는 시간이 짧아지면서 더 많은 시도를 할 수 있었고, 그만큼 성과와 시행착오도 빠르게 쌓였다.

AI는 팀 내부의 시간도 가속했다. 맡아야 할 일의 범위와 우선순위, 구성원에게 필요한 역할이 빠르게 달라졌다. 팀 안의 변화를 함께 살피고 업무 분담과 협업 방식을 능동적으로 조정하는 것이 무엇보다 중요한 환경이 됐다.

나는 익숙하지 않은 기술로 만든 레거시를 수습하고 혼자 개발을 이어 갈 수 있었고, 대표는 개발 경험 없이도 자신의 사업에 필요한 서비스를 직접 만들기 시작했다. AI는 두 사람 모두에게 이전에는 감당하기 어려웠던 일을 할 수 있게 해줬다.

### 빠른 성과 뒤에 남은 복잡도

그 과정에서 기존 팀에서는 AI의 완료 보고를 검토 없이 전달하는 일이 반복됐고, 나도 AI가 잘할수록 더 큰 일을 맡기며 확인을 뒤로 미뤘다. 대표는 직접 만들어 낸 결과가 쌓일수록 AI의 제안을 더 신뢰했다. 눈앞의 성공이 다음 결과까지 믿을 이유가 되는 점은 닮아 있었다.

특히 개발을 잘 모르는 상태에서 AI로 생각했던 결과물을 빠르게 쏟아내다 보면 무엇이든 할 수 있을 것 같은 만능감에 빠지기 쉽다. 원하는 화면과 기능이 계속 만들어지니 전문가의 조언 따위 없어도 아쉽지 않다. 그 결과물이 얼마나 제대로 만들어졌는지 판단할 기준이 부족하다는 사실은 눈앞의 성과에 가려진다.

이 프로젝트에서는 대규모 리팩터링을 하고도 복잡도가 줄지 않았다. Supabase의 DB 함수에 있던 로직을 백엔드로 옮겼을 뿐인데, 대표는 구조까지 정리됐다고 믿고 있다. 개별 작업을 많이 끝냈다는 사실이 시스템 전체가 나아졌다는 판단으로 이어진 것이다.

이 상태에서 계속 기능을 늘리면 한계가 드러나는 것은 시간문제라고 생각한다. 처음에는 막대한 토큰 비용이 부담되기 시작한다. 그러다 오류가 점점 잦아지고, 에이전트는 원래 의도와 다른 기능을 구현하기 시작한다. 나는 결국 다시 만드는 상황에 이르게 될 것이라고 본다. 사람이 개발할 때도 겪었던 일이다. AI라고 다르지 않다.

### 개발자에게 남는 분석·설계·검증

대표에 대한 내 감정을 뒤로하고 보면, 이 사례는 비개발자가 AI로 개발할 때 개발자가 어떤 역할을 할 수 있는지를 보여준다. 최근 Codex 6.0 Astra로 개발해 보면서도, 문제를 분석하고 설계 전략을 세우는 데는 사람의 논리와 경험이 거의 필수처럼 느껴졌다. 분석과 설계안 작성도 AI에게 맡길 수 있지만, 그 제안에서 빠진 전제를 찾고 어떤 경계로 나눌지, 어떤 순서로 바꿀지 판단하는 일이 남았다.

여러 에이전트가 동시에 일할수록 도메인과 인터페이스의 경계를 정하고 각 작업의 의존성을 줄이는 일이 필요했다. 사람이 나눠 개발할 때 사용하던 소프트웨어 공학의 원리는 AI에게 일을 맡길 때도 유효했다. 동시번역처럼 사용 목적에 맞는 동작 방식을 정하는 일이나, 테스트가 무엇을 확인하는지 살피는 일에도 개발자의 판단이 필요했다.

내가 생각하는 개발자의 역할은 비개발자와 이런 판단을 함께 하며 개발 방향을 잡고, 그 결과가 실제로 문제를 해결했는지 확인하는 것이다.

<a id="trust-and-communication"></a>

### 구현이 빨라질수록 협력은 더 중요해졌다

구현에 많은 시간이 필요했던 과거에는 각자 맡은 일을 진행하는 시간이 전체 개발 기간에서 큰 비중을 차지했다. 서로의 설명이나 결정을 기다리는 시간이 상대적으로 덜 두드러질 수 있었다. 그러나 구현 시간이 극단적으로 줄어들면 그 기다림과 조율이 프로젝트 전체의 병목이 된다. 각자가 빠르게 일을 끝내도 서로 다른 전제로 만든 결과를 연결하지 못하면 완성은 계속 미뤄진다.

개발자와 비개발자는 더 긴밀하게 협력해야 하는 상황이 됐다. 사업의 의도와 현장의 조건, 기술적인 제약과 선택의 근거를 함께 검토하고, 달라진 우선순위와 역할을 수시로 맞춰야 했다. 각자 자기 일만 빨리 처리하는 것으로는 AI가 줄여 준 시간을 프로젝트 전체의 성과로 이어 갈 수 없었다.

나도 사람에게 설명하고 조율하는 일이 부담스러워 AI와 직접 개발하는 편을 택했다. 개발자와 협업하며 피로와 불신이 쌓인 비개발자라면, 사람에 대한 의존을 줄일 수 있다는 점에서도 AI가 매력적일 것이다. 대표가 직접 개발하는 선택을 이해할 수 있는 이유다. 그러나 사람을 덜 고용해도 요구사항을 정하고 결과를 판단할 책임은 남았다.

이 프로젝트에서 가장 중요하고 필요했던 것은 인간관계였다. 기존 팀장과 대표 사이에서도, 대표와 나 사이에서도 상호 신뢰를 구축하는 데 실패했다. 각자가 자기 판단에 따라 진행한 작업이 하나의 프로젝트를 완성하는 방향으로 충분히 연결되지 못했다. 그런 개인플레이가 프로젝트 완성의 큰 장애물이 되었다.

나는 출근 전부터 신뢰가 중요하다고 생각했고 첫날부터 그 뜻을 행동으로 보여주려 했다. 레거시를 수습하고 4월의 별도 행사 준비에서 겪은 위기도 잘 넘겼다고 생각했다. 그럼에도 대표는 끝까지 나보다 AI를 신뢰했다. 이 분야의 전문가보다 AI를 더 신뢰해서 생긴 일이라고 느꼈다. 혹은 사람을 신뢰하지 못하기 때문에 벌어진 일인지도 모르겠다.

나 역시 첫날부터 팀장에게 냉랭하게 대했고, 나중에는 대표와의 관계가 어긋나자 더 나은 의견을 내지 않게 됐다. 공감과 커뮤니케이션이 중요하다는 것을 아는 것과, 실제로 서로의 판단을 함께 검토하고 작업을 연결할 수 있는 관계를 유지하는 것은 달랐다.

대표는 개발자의 오만함을 이야기했지만, 개발자도 대표의 야박함 때문에 마음을 닫을 수 있다는 점은 잘 보지 못하는 것 같았다. 개발자는 대표의 야박함을, 대표는 개발자의 오만함을 보며 서로 멀어져 간다. 처음 만난 팀장의 태도도 그런 맥락에서 돌아보게 된다. 그에게 어떤 사정이 있었는지는 모르지만, 나도 관계에 따라 일하는 태도가 달라졌기 때문이다.

이런 감정적인 이야기를 구체적으로 꺼내는 것은 협력이 어디에서 끊겼는지 보여주기 위해서다. 신뢰가 무너지자 각자가 알고 있는 것과 할 수 있는 일이 하나의 프로젝트 안에서 충분히 만나지 못했다.

서로의 입장을 더 잘 이해할 수 있기를 바라는 마음도 있다. 상대방이 하는 일이 부족해 보여도, 막상 내가 그 입장에서 해 보면 별반 다르지 않을 수 있다. 각자 맡은 일에는 밖에서 잘 보이지 않는 어려움이 있다. 그 어려움을 함께 이해하고 변화에 대응할 수 있는 관계를 만드는 일도 개발을 완성하는 데 필요했다.

나도 이 회사에서 열심히 일했다고 생각한다. 그래도 누군가에게는 나쁜 개발자가 되어 있을지도 모른다.

다음 글에서는 이 경험을 바탕으로 개발자가 어떤 판단과 검증을 맡아야 하는지, 여러 에이전트와 어떻게 일할지, 그 능력을 기르려면 어떤 경험과 학습 조건이 필요한지 정리하려고 한다.
